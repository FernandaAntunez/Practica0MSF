...
R,L,C = 10E3, 47E-3, 220E-6
num = [1]
den = [C*L, C*R, 1]
sys = ctrl.tf(num,den)
print (f"Función de tranferencia del sistema : {sys}")
# Componentes del controlador
kp, kI, kD = 289.1654,7037.5609, 0.39274
Cr = 1e-6
Re = 1/(kI*Cr)
Rr = kp*Re
Ce = kD/Rr
...
plt.xlim(0,10); plt.xticks(np.arange(0,11,1))
plt.ylim(-1.2,1.); plt.yticks(np.arange(-1.2,1.4,0.2))
plt.xlabel('t [s]', fontsize = 11)
plt.ylabel ('Vi(t) [V]', fontsize = 11)
plt.legend(bbox_to_anchor = (0.5, -0.2), loc= 'center', ncol=3,
           fontsize = 9, frameon = True)
